...
bool Gui::Initialize() {
    ...
    HWND hwnd = CreateWindowExA(0, "WireMergeWindowClass", kWireMergeDisplayTitle,
    ...
    PushLogLine("This is the Stripped Build. It is fully functional; the only "
                "difference from the regular build is that it does not download "
                "Android capture tools automatically. If tools/adb.exe or "
                "tools/sndcpy.apk are ever lost, get a fresh copy of the Stripped "
                "Build from the GitHub repository rather than expecting in-app recovery.");
    showStrippedBuildNotice_ = true;
...
void Gui::RenderToolbar(float& outContentY) {
    ...
    ImGui::TextUnformatted(kWireMergeDisplayTitle);
...
void Gui::RenderDevicesContent(const PaneRenderContext& /*ctx*/) {
    ...
    if (androidExpanded) {
        ImGui::Dummy(ImVec2(0, 4.0f));
        if (!adb_.IsAvailable()) {
            ImGui::TextWrapped("Not set up: adb.exe / sndcpy.apk missing from 'tools'.");
            ImGui::Dummy(ImVec2(0, 4.0f));
            ImGui::TextWrapped("Get adb.exe from Android's platform-tools and sndcpy.apk "
                                "from the sndcpy GitHub releases, then place both in a "
                                "'tools' folder next to WireMerge.exe. See Log for direct links.");
    ...
    if (!loggedInitialToolsCheck_ && adb_.IsAsyncInitDone()) {
        ...
        if (adb_.IsAvailable()) {
            ...
            PushLogLine("Android capture tools not found in tools/. Android capture is unavailable "
                        "until installed manually (see Devices panel for where to get them).");
        }
    }
}
...
void Gui::RenderFrame() {
    ...
    if (showStrippedBuildNotice_) {
        ImGui::OpenPopup("Stripped Build");
        showStrippedBuildNotice_ = false;
    }
    ImGui::SetNextWindowPos(ImGui::GetMainViewport()->GetCenter(), ImGuiCond_Appearing, ImVec2(0.5f, 0.5f));
    if (ImGui::BeginPopupModal("Stripped Build", nullptr,
                                ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoSavedSettings)) {
        RegisterFooterOccluder();
        ImGui::PushTextWrapPos(ImGui::GetCursorPosX() + 380.0f);
        ImGui::TextWrapped("You're running the Stripped Build. It is fully functional; "
                            "the only difference from the regular build is that it does "
                            "not download Android capture tools automatically.");
        ImGui::TextWrapped("If tools/adb.exe or tools/sndcpy.apk are ever lost, get a "
                            "fresh copy of the Stripped Build from the GitHub repository "
                            "rather than expecting the app to fetch them for you.");
    ...
    if (ImGui::BeginPopupModal("Licenses", nullptr,
        ...
        ImGui::TextWrapped("WireMerge is not code-signed. Signing through SignPath.io could "
                            ...
                            "the regular build.");
        ImGui::TextWrapped("You are currently running the Stripped Build, which removes the "
                            "automatic Android-tool download feature specifically, since that "
                            "behavior is what appears to trigger most of those flags. Everything "
                            "else is functionally identical to the regular build.");
        ImGui::PopTextWrapPos();
        ImGui::Dummy(ImVec2(0, 10.0f));
        ImGui::Separator();
        ImGui::Dummy(ImVec2(0, 10.0f));
    ...
    bool anyFooterModalOpen = ImGui::IsPopupOpen("Licenses", ImGuiPopupFlags_None) ||
                               ...
                               ImGui::IsPopupOpen("Stripped Build", ImGuiPopupFlags_None);
